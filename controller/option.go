...
func UpdateOption(c *gin.Context) {
	...
	case service.ErrorRewriteSourceKey:
		...
		if source != "local" && source != "http" && source != "sql" {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "error rewrite source must be local, http, or sql"})
			return
		}
	case service.ErrorRewriteMonitorRulesJSONKey:
		if _, parseErr := service.ParseErrorRewriteMonitorRulesJSON(option.Value.(string)); parseErr != nil {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "error rewrite monitor rules JSON is invalid: " + parseErr.Error()})
			return
		}
	case service.ErrorRewriteSQLDriverKey:
		...
		if driver != "mysql" && driver != "postgres" && driver != "postgresql" && driver != "sqlite" {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "error rewrite SQL driver must be mysql, postgres, or sqlite"})
			return
		}
	case service.ErrorRewriteSQLQueryKey:
		...
		if query != "" && !strings.HasPrefix(query, "select") {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "error rewrite SQL query must be a SELECT statement"})
			return
		}
	case service.ErrorRewriteRefreshSecondsKey:
		...
		if parseErr != nil || refreshSeconds < 1 {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "error rewrite refresh interval must be greater than 0"})
			return
		}
	case service.ErrorRewriteRequestTimeoutMSKey:
		...
		if parseErr != nil || timeoutMS < 100 {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "error rewrite request timeout must be at least 100ms"})
			return
		}
