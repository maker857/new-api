...
func TestNativeVolcengineASRRejectsNonVolcengineChannel(t *testing.T) {
	...
	c.Set(string(constant.ContextKeyChannelOtherSetting), dto.ChannelOtherSettings{
		VolcASR: &dto.VolcASRConfig{Protocol: dto.VolcASRProtocolV3AUC, ResourceID: "volc.seedasr.auc", AuthMode: dto.VolcASRAuthModeNewConsole},
	})
...
func TestNativeVolcengineASRRejectsChannelResourceMismatch(t *testing.T) {
	...
	c.Set(string(constant.ContextKeyChannelKey), "channel-key")
	c.Set(string(constant.ContextKeyChannelOtherSetting), dto.ChannelOtherSettings{
		VolcASR: &dto.VolcASRConfig{Protocol: dto.VolcASRProtocolV3AUC, ResourceID: "volc.bigasr.auc", AuthMode: dto.VolcASRAuthModeNewConsole},
	})
	info := &relaycommon.RelayInfo{
		RelayMode:       relayconstant.RelayModeVolcengineASRNative,
		OriginModelName: "volc.seedasr.auc",
