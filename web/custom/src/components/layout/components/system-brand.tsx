...
export function SystemBrand(props: SystemBrandProps) {
  ...
  const { logo, logoLoaded, loading } = useSystemConfig()

  const variant = props.variant ?? 'sidebar'
  const name = props.defaultName || 'Kalos'
  const version = props.defaultVersion || 'AI Gateway'
  const customLogo = Boolean(logo && logo !== DEFAULT_LOGO)

  const mark = (
    <span
      aria-hidden='true'
      className='bg-foreground text-background flex size-full items-center justify-center rounded-[inherit] font-semibold tracking-normal'
    >
      K
    </span>
  )
  const logoMark =
    customLogo && logoLoaded && !loading ? (
      <img
        src={logo}
        alt={name}
