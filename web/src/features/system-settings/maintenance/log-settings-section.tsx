...
function getDiagnosticStorageDisplay(bytes: number) {
  ...
  return {
    value: Math.floor(bytes / diagnosticStorageMB),
    unit: "MB" as const,
  };
}
...
function ruleChannelLabel(rule: ErrorRewriteMonitorRule): string {
  ...
  const id =
    rule.channel_id && rule.channel_id > 0 ? `#${rule.channel_id}` : "";
...
export function LogSettingsSection({
  ...
  }, [fetchMonitorBlacklistRules, fetchServerLogInfo, mode]);
                ...
                <div className="bg-background space-y-3 rounded-xl border p-4">
                  <div>
                    <h4 className="text-sm font-medium">
                      {t("Capture storage and paths")}
                    </h4>
                    <p className="text-muted-foreground mt-1 text-xs">
                      {t(
                        "Choose where captures are saved and which relay paths are recorded.",
                      )}
                    </p>
                  </div>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <div className="space-y-2">
                          ...
                          <InputGroup>
                            <InputGroupAddon className="bg-muted/50 max-w-[60%] shrink-0 truncate border-r pr-2">
                              {diagnosticStorageInfo?.storage_root
                                ? `${diagnosticStorageInfo.storage_root.replace(/[\\/]+$/, "")}/`
                                : t("Loading...")}
                            </InputGroupAddon>
                            <FormControl>
                              <InputGroupInput
                          ...
                          <FormDescription>
                            {t(
                              "The gray path is the application storage root and cannot be changed here. Edit only the directory after it.",
                            )}
                          </FormDescription>
                          <FormMessage />
                        </div>
                ...
                <div className="space-y-4 border-t pt-5">
                  ...
                  <div className="bg-background space-y-3 rounded-xl border p-4">
                    <div>
                      ...
                      <p className="text-muted-foreground mt-1 text-xs">
                        {t(
                          "Each type becomes eligible for cleanup independently.",
                        )}
                      </p>
                    </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-[minmax(0,1fr)_8rem] gap-2">
                              <Input
                                ...
                                min={0}
                                max={
                                  minRetentionUnit === "hours" ? 87600 : 5256000
                                }
                                step={1}
                                value={
                            ...
                            <FormDescription>
                              {t(
                                "A completed capture is protected for this long before automatic cleanup. Set 0 to allow cleanup immediately when storage is needed.",
                              )}
                            </FormDescription>
                            <FormMessage />
                          </div>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <div className="space-y-2">
                            ...
                            <FormDescription>
                              {t(
                                "An incomplete capture is eligible for cleanup only after this duration. Set 0 to keep it indefinitely.",
                              )}
                            </FormDescription>
                            <FormMessage />
                          </div>
